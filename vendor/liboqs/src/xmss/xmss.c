...
// Like pkey_xmss_ctrl() but with string commands
static int pkey_xmss_ctrl_str(EVP_PKEY_CTX *ctx,
...
void load_pmeth()
{
	...
	static EVP_PKEY_METHOD x =
	{
		...
		pkey_xmss_ctrl_str // pkey_xmss_ctrl_str
	};
...
static int xmss_cms_sign(CMS_SignerInfo *si)
{
    X509_ALGOR *alg;
    EVP_PKEY_CTX *pkctx = CMS_SignerInfo_get0_pkey_ctx(si);
    (void)(pkctx);
    CMS_SignerInfo_get0_algs(si, NULL, NULL, NULL, &alg);
    X509_ALGOR_set0(alg, OBJ_nid2obj(NID_xmss), V_ASN1_NULL, 0);
    return 1;
}

static int xmss_cms_verify(CMS_SignerInfo *si)
{
    X509_ALGOR *alg;
    EVP_PKEY_CTX *pkctx = CMS_SignerInfo_get0_pkey_ctx(si);
    (void)(pkctx);
...
static int xmss_pkey_ctrl(EVP_PKEY *pkey, int op, long arg1, void *arg2)
{
    (void)(pkey);
    X509_ALGOR *alg = NULL;
    switch (op) {
    ...
    case ASN1_PKEY_CTRL_CMS_SIGN:
        if (arg1 == 0)
            return xmss_cms_sign(arg2);
        else if (arg1 == 1)
            return xmss_cms_verify(arg2);
        break;
